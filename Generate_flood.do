...
drop farmval
sort fips
merge fips using out_migrant_counts.dta
drop if _merge==2
drop _merge
...
rename var173 mules
sort state county
merge state county using icpsr_fips.dta
keep if _merge==3
...
insheet using Haines_1925_new.txt
sort state county
...
*New Data from Haines
insheet using Haines_1930_new.txt
sort state county
drop if state==0|state==.|state>73
...
*Data from Rhode
insheet using redcross_new.txt
...
drop _merge
sort fips
merge fips using in_migrant_counts.dta
tab _merge
drop if _merge==2
drop _merge
sort fips
...
*New Data from Haines
insheet using Haines_1935_new.txt
sort state county
drop if state==0|state==.|state>73
...
*New Data from Haines
insheet using Haines_1940_new.txt
sort state county
drop if state==0|state==.|state>73
destring oats_y, replace
...
use "usag1969.cos.crops.dta", clear
...
use "usag74.1969.allfarms.work.dta", clear
keep if state==32|state==34|state==40|state==41|state==42|state==43|state==44|state==45|state==46|state==47|state==48|state==49|state==51|state==52|state==53|state==54
...
insheet using Export1910_1900.txt, tab
...
insheet using Export1920_1900.txt, tab
keep if state==state_1
gen percent = new_area/area
rename id fips
sort fips
...
insheet using Export1930_1900.txt, tab
keep if state==state_1
gen percent = new_area/area
rename id fips
sort fips
...
insheet using Export1940_1900.txt, tab
keep if state==state_1
gen percent = new_area/area
rename id fips
sort fips
...
insheet using Export1950_1900.txt, tab
keep if state==state_1
gen percent = new_area/area
rename id fips
sort fips
...
insheet using Export1960_1900.txt, tab
keep if state==state_1
gen percent = new_area/area
rename id fips
sort fips
...
insheet using Export1970_1900.txt, tab
keep if state==state_1
gen percent = new_area/area
rename id fips
sort fips
...
insheet using flooded_1900.txt, tab
...
insheet using distance_1900.txt, tab
sort fips
save `distance_1900', replace
clear all

*Prepare distance to MS data
insheet using ms_distance.txt, tab
...
*Prepare ruggedness
use 1900_strm_distance_gaez.dta
...
rename id fips
sort fips
merge fips using brannenplantcounties_1910.dta
keep if _merge==3
drop _merge
...
use new_deal_spending.dta, clear
keep if state==32|state==34|state==40|state==41|state==42|state==43|state==44|state==45|state==46|state==47|state==48|state==49|state==51|state==52|state==53|state==54
